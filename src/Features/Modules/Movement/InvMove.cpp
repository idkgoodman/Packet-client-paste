...
DEFINE_PATCH_FUNC(InvMove::patchFunc1, SigManager::PlayerMovement_clearInputStateInlined, gClrBytes);
std::vector<unsigned char> gClrBytes2 = { 0x90, 0x90, 0x90, 0x90, 0x90, 0x90 };
DEFINE_PATCH_FUNC(InvMove::patchFunc2, SigManager::PlayerMovement_clearInputStateInlined2, gClrBytes);


void InvMove::onEnable()
{
    gFeatureManager->mDispatcher->listen<BaseTickEvent, &InvMove::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->listen<RenderEvent, &InvMove::onRenderEvent>(this);
    gFeatureManager->mDispatcher->listen<PacketInEvent, &InvMove::onPacketInEvent>(this);
    gFeatureManager->mDispatcher->listen<PacketOutEvent, &InvMove::onPacketOutEvent>(this);
    patchFunc(true);
...
{
    gFeatureManager->mDispatcher->deafen<BaseTickEvent, &InvMove::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->deafen<RenderEvent, &InvMove::onRenderEvent>(this);
    gFeatureManager->mDispatcher->deafen<PacketInEvent, &InvMove::onPacketInEvent>(this);
    gFeatureManager->mDispatcher->deafen<PacketOutEvent, &InvMove::onPacketOutEvent>(this);
    patchFunc(false);
    auto player = ClientInstance::get()->getLocalPlayer();
...
void InvMove::onBaseTickEvent(BaseTickEvent& event)
{
...
void InvMove::onPacketInEvent(PacketInEvent& event)
{
...
void InvMove::onPacketOutEvent(PacketOutEvent& event)
{
    if (event.mPacket->getId() == PacketID::ContainerClose)
    {
        mHasOpenContainer = false;
