...
void Regen::renderFakeOres() {
    ...
    void onRenderEvent(RenderEvent& event) {
        ...
        // Позиция прогресс-бара - 40 пикселей над инвентарем (предполагаем, что инвентарь находится внизу)
        ImVec2 pos = ImVec2(10, 500); // Примерная позиция, можно настроить по необходимости
        ImVec2 size = ImVec2(200, 30); // Размер прогресс-бара

        // Размытие фона
        ImVec4 area = ImVec4(pos.x, pos.y, pos.x + size.x, pos.y + size.y);
        ImRenderUtils::addBlur(area, 10.f, 10.f, drawList, false); // Размытие внутри

        // Тени вокруг
        drawList->AddShadowRect(ImVec2(pos.x - 3, pos.y - 3), ImVec2(area.z + 3, area.w + 3), ImColor(0.f, 0.f, 0.f, 1.f), 50.f, ImVec2(0, 0));

        // Прогресс-бар с цветом темы
        ImU32 themedColor = ColorUtils::getThemedColor(0); // Получаем цвет из темы
        ImVec2 progressSize = ImVec2(size.x * progress, size.y); // Прогресс
        drawList->AddRectFilled(pos, ImVec2(pos.x + progressSize.x, pos.y + progressSize.y), themedColor, 10.0f); // Заполнение прогресса

        // Отображение текста "Mining %"
        std::string progressText = "Mining " + std::to_string(int(progress * 100)) + "%";
        ImVec2 textPos = ImVec2(pos.x + 5, pos.y + 5); // Позиция текста внутри полоски
        ImRenderUtils::drawShadowText(drawList, progressText, textPos, ImColor(255, 255, 255, 255), 20.f, false); // Рисуем текст
    }

    void onBaseTickEvent(BaseTickEvent& event) {
        // Это можно использовать для дополнительного обновления прогресса
        // Например, если нужно добавить логику для отслеживания разных состояний.
    }

    void onBlockBreakEvent(BlockBreakEvent& event) {
        if (event.isBreakingBlock()) {
            // Получаем прогресс разрушения
            float blockProgress = event.getProgress(); // Получаем прогресс разрушения блока
            progress = blockProgress; // Обновляем прогресс

            // Можно добавить дополнительные условия для отображения только если блок разрушен частично
        }
    }

private:
    float progress = 0.0f; // Прогресс разрушения (от 0.0 до 1.0)
};
