...
class Manager : public ModuleBase<Manager> {
public:
    enum class Mode {
        Instant,
        Delayed
    ...
    Manager() : ModuleBase("Manager", "Manages your inventory", ModuleCategory::Player, 0, false) {
        addSetting(&mMode);
        addSetting(&mDelay);
        ...
        mNames = {
            {Lowercase, "manager"},
            {LowercaseSpaced, "manager"},
            {Normal, "Manager"},
            {NormalSpaced, "Manager"}
        };
    }
