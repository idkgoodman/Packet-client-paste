...
#include <Features/Modules/Render/Interface.hpp>
...
bool OreBreaker::isValidBlock(glm::ivec3 blockPos, bool oreOnly, bool exposedOnly, bool usePriority, OrePriority priority) {
    auto player = ClientInstance::get()->getLocalPlayer();
...
void OreBreaker::queueBlock(glm::ivec3 blockPos) {
    Block* block = ClientInstance::get()->getBlockSource()->getBlock(blockPos);
...
OreBreaker::PathFindResult OreBreaker::getBestPathToBlock(glm::ivec3 blockPos) {
    auto player = ClientInstance::get()->getLocalPlayer();
...
bool OreBreaker::isOre(std::vector<int> Ids, int id)
{
    return std::find(Ids.begin(), Ids.end(), id) != Ids.end();
}

void OreBreaker::onEnable()
{
    gFeatureManager->mDispatcher->listen<BaseTickEvent, &OreBreaker::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->listen<PacketOutEvent, &OreBreaker::onPacketOutEvent, nes::event_priority::VERY_LAST>(this);
    gFeatureManager->mDispatcher->listen<RenderEvent, &OreBreaker::onRenderEvent>(this);
...
void OreBreaker::onDisable()
{
    gFeatureManager->mDispatcher->deafen<BaseTickEvent, &OreBreaker::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->deafen<PacketOutEvent, &OreBreaker::onPacketOutEvent>(this);
    gFeatureManager->mDispatcher->deafen<RenderEvent, &OreBreaker::onRenderEvent>(this);
...
void OreBreaker::onBaseTickEvent(BaseTickEvent& event)
{
...
void OreBreaker::onRenderEvent(RenderEvent& event)
{
    auto player = ClientInstance::get()->getLocalPlayer();
    if (!player) return;
...
void OreBreaker::onPacketOutEvent(PacketOutEvent& event)
{
    auto player = ClientInstance::get()->getLocalPlayer();
    if (!player) return;
