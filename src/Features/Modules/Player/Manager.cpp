...
void Manager::onEnable()
{
    gFeatureManager->mDispatcher->listen<BaseTickEvent, &Manager::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->listen<PacketInEvent, &Manager::onPacketInEvent>(this);
    gFeatureManager->mDispatcher->listen<PacketOutEvent, &Manager::onPacketOutEvent>(this);
    gFeatureManager->mDispatcher->listen<PingUpdateEvent, &Manager::onPingUpdateEvent>(this);
}

void Manager::onDisable()
{
    gFeatureManager->mDispatcher->deafen<BaseTickEvent, &Manager::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->deafen<PacketInEvent, &Manager::onPacketInEvent>(this);
    gFeatureManager->mDispatcher->deafen<PacketOutEvent, &Manager::onPacketOutEvent>(this);
    gFeatureManager->mDispatcher->deafen<PingUpdateEvent, &Manager::onPingUpdateEvent>(this);
}

void Manager::onBaseTickEvent(BaseTickEvent& event)
{
...
void Manager::onPacketInEvent(PacketInEvent& event)
{
...
void Manager::onPacketOutEvent(PacketOutEvent& event)
{
    if (event.mPacket->getId() == PacketID::ContainerClose)
    {
        mHasOpenContainer = false;
...
void Manager::onPingUpdateEvent(PingUpdateEvent& event)
{
    mLastPing = event.mPing;
}

bool Manager::isItemUseless(ItemStack* item, int slot)
{
    if (!item->mItem) return true;
    auto player = ClientInstance::get()->getLocalPlayer();
    SItemType itemType = item->getItem()->getItemType();
    auto itemValue = ItemUtils::getItemValue(item);
    auto Manager = gFeatureManager->mModuleManager->getModule<InvManager>();
